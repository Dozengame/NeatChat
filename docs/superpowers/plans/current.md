...
- Branch: `dev`.
- Current HEAD: `abedf196 Add configurable reasoning effort allowlists`.
- Composer-only model selection is implemented locally but not committed: empty new chat, unsent draft, existing chat, desktop, and compact layouts now share one selector inside the composer; no push, PR, deploy, or remote configuration change was performed.
...
- GPT-5.x reasoning visual work remains approval-gated. Three design-only directions were generated from the current NeatChat screenshots and the `zanwei/chatgpt-model-selector` reference: a six-stop Intelligence Rail, a 3x2 Reasoning Cards grid, and a drill-in Focus Mode. No reasoning UI code, wire value, allowlist, locked-state, or animation behavior changed in this slice.

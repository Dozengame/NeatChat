...
Server-side work without a UI surface does not require Browser QA. Use targeted Jest, logs, status/headers/payload evidence, `lint`, `tsc`, and build instead. Local-only documentation changes use formatting, content, ignore, and relevant compatibility checks rather than a full application build.

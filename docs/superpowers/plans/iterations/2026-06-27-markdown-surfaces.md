...
## 2026-07-11 Adaptive Markdown Surfaces And Final Visual Polish

- Product commit: `b7a56703 fix(markdown): refine adaptive content surfaces`.
- Width policy is now two-state and evidence-based: table, unwrapped code, Mermaid SVG, and HTML Preview start on the centered `780px` prose rail and promote once to the centered `920px` rail only when real rendered width exceeds `780px + 12px`. Content changes reset the relevant state; mobile/constrained layouts remain capped by the available parent width and never create document overflow.
- Table semantics and interaction now use logical start alignment by default while preserving explicit GFM alignment, default `scope="col"`, strict-only compact-cell list conversion, real table plus viewport observation, overflow-only labeled keyboard regions, a first-scroll-dismissed mobile hint, neutral hover vs focus emphasis, and no duplicate final-row separator.
- Remaining visual findings were closed in the same slice: consecutive `hr` elements keep `10px` internal rhythm; HTML Preview caption/frame form one attached chrome surface without injecting theme styles into the user iframe; scroll-to-bottom uses a real opaque/faded safe lane at constrained/mobile sizes and moves into the wide-canvas right gutter at a `1032px` chat-container breakpoint.
- Artifact metrics preserve the previous `body.clientHeight` auto-height behavior, use `window.innerWidth` for width comparison so vertical scrollbars do not suppress promotion, and retain source/frame-id/finite-positive validation. Mermaid measures the generated SVG viewBox. Code/table/artifact width state is monotonic within one content identity and reset by structural content identity.
- Verification after the final commit hook: focused width/table/code/Mermaid/artifact plus visual contracts passed `6/6` suites and `107/107` tests; the broader relevant matrix passed `15/15` suites and `159/159` tests; ESLint, TypeScript, `git diff --check`, production build, and independent reviewer all passed.
- Chrome QA used real compiled product CSS because the local Chat route was legitimately access-code gated. At `1650x941`, normal/wide surfaces measured `780/920px` on one center line; at `1056x834`, wide safely capped at `916px`; at `390x844` and `320x740`, normal/wide collapsed to identical centered `346px`/`276px` widths. All four viewports had zero document overflow. Mobile table hint was `30px`, the edge fade ended exactly at the scroll viewport instead of covering the hint, and the table kept `344/274px` internal viewports with `913/897px` scroll widths.
- Safe-lane runtime geometry was `66px` with a `42px` button and `12px` gaps at constrained desktop, and `60px` with a `40px` button and `10px` gaps at mobile/narrow. Desktop gutter placed the button outside the `920px` canvas. Light/Dark row hover and focus computed to distinct states; artifact caption/frame borders matched on hover; a fresh fixture tab and the already-open authenticated NeatChat tab both reported zero console warning/error.
- Boundary: the local-only visual harness and screenshots remain under ignored `.codex/`; they were not staged. No access control bypass, credential read, paid request, push, PR, or deployment occurred.

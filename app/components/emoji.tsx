...
function containsPattern(value: string, pattern: string) {
  const normalizedPattern = pattern.toLowerCase();
  return (
    normalizedPattern.length > 0 && value.split(normalizedPattern).length > 1
  );
}
  ...
  try {
    ...
    for (const [category, pattern] of Object.entries(categoryPatterns)) {
      ...
      for (const p of patterns) {
        if (containsPattern(lowerModelId, p)) {
          return category;
        }
      }
    }
